...
const storage = global?.temp?.g4o || new Map();
...
const gcName = (await threadsData.get(event.threadID)).threadName;
  try {
    ...
    global.GoatBot.onReply.set(replied.messageID, {
    ...
    console.error(error);
    message.reply(formatFont(`Error: ${error.message}`));
...
module.exports.config = {
  ...
  author: "Jsus",
  coolDown: 5,
  description: { en: "A watered down version of Gemini4o" },
  category: "ai",
  guide: { en: "{pn} <prompt>" }
};
